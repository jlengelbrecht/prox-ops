...
from flagger_recovery.identity import AttributionRefused, resolve
...
class DecisionLoggingTests(ReceiverTestCase):
    """F16: one INFO decision line per handled hook, so a stored write, a
    swallowed duplicate and an ``Ignore`` are distinguishable from the access
    line alone."""

    LOGGER = "flagger_recovery.server"

    def test_registered_and_duplicate(self):
    ...
    def test_refuse_and_propose_correction_from_an_injected_decider(self):
        ...
        receiver = Receiver(
            token=TOKEN, store=self.store, candidates=self.candidates,
            decider=lambda record, event: Decision(kind="ProposeCorrection", reason=proposal.summary, proposal=proposal),
        )
        with self.assertLogs(self.LOGGER, level="INFO") as logs:
    ...
    def test_never_logs_metadata_and_clamps_a_hostile_optional_field(self):
        ...
        length- and whitespace-trimmed at intake, not character-restricted --
        are reclamped through ``label_value``/``checksum_label`` before they
        reach the line, so a newline or a forged ``key=value`` cannot ride in."""
        with self.assertLogs(self.LOGGER, level="INFO") as logs:
            self.post("pre-rollout", _body(
                metadata={"token": TOKEN, "secret": "do-not-log-me"},
                phase="fake\nhook=post-rollout result=Injected",
            ))
        line = logs.output[-1]
